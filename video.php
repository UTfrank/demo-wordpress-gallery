...
<head>
	...
	<title><?php _e('Justified Image Grid video player', 'jig_td'); ?>
	</title>
	<link rel='stylesheet' id='mediaelement-css'
		href='<?php echo includes_url("js/mediaelement/mediaelementplayer.min.css"); ?>' type='text/css' media='all' />
	<link rel='stylesheet' id='wp-mediaelement-css'
		href='<?php echo includes_url("js/mediaelement/wp-mediaelement.css"); ?>' type='text/css' media='all' />
		...
		jq.onload = function() {
			...
			meap.onload = function() {
				...
				wpme.onload = function() {
					...
					jigv.src =
						'<?php echo plugins_url('js/jig-video' . $dotmin . '.js', __FILE__); ?>';
					head.appendChild(jigv);
				}
				wpme.src =
					'<?php echo includes_url("js/mediaelement/wp-mediaelement.js"); ?>';
			}
			meap.src =
				'<?php echo includes_url("js/mediaelement/mediaelement-and-player.min.js"); ?>';
		}
		jq.src =
			'<?php echo includes_url("js/jquery/jquery.js"); ?>';
	}(document, 'script'));
	</script>
	...
	.mejs-embed body {
		background:
			<?php echo $jig_settings['video_area_background'];
		?>;
		/* player background */
	}
...
<body>
	<video id="jigVideoPlayer" height="100%" width="100%"
		<?php echo !empty($poster) ? 'poster="' . $poster . '"' : ''; ?> src="<?php echo $file; ?>">
	</video>
</body>
