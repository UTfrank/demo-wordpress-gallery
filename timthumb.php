 ...
 * WebP support courtesy of mbariola
 *
 * $Rev$
 */
...
class timthumb
{
    ...
    protected $filePrependSecurityBlock = "<?php die('Execution denied!'); //"; //Designed to have three or four letter mime type, space, question mark and greater than symbol appended. 7 bytes total for webp/avif and 6 for all other handled types.
    ...
    protected function processImageAndWriteToCache($localImage)
    {
        ...
        if (! preg_match('/^image\/(?:gif|jpg|jpeg|png|webp|avif)$/i', $mimeType)) {
            return $this->error("The image being resized is not a valid gif, jpg, png, webp or avif.");
        }
        ...
        // Firsh: .webp ext in the f param forces webp output
        // URL crafter's responsibility to ensure that the browser supports it
        if (function_exists('imageavif') && ($this->param('f') === '.avif' || preg_match('/^image\/avif$/i', $mimeType))) {
            $imgType = 'avif';
            // https://www.industrialempathy.com/posts/avif-webp-quality-settings/
            imageavif($canvas, $tempfile, ceil($quality * 0.8));
        } elseif ($this->param('f') === '.webp' || preg_match('/^image\/webp$/i', $mimeType)) {
            $imgType = 'webp';
            imagewebp($canvas, $tempfile, $quality);
        } else {
            //Straight from Wordpress core code. Reduces filesize by up to 70% for PNG's
            // Moved here to unaffect nextgen formats
    ...
    {
        $ngOffset = 0; // this is either 0 for non next-gen files, or 1 for webp/avif files.
        ...
        $lowImgType = strtolower($imgType);
        if ($lowImgType === 'webp' || $lowImgType === 'avif') {
            $ngOffset = 1;
        } else {
        ...
        fseek($fp, 3, SEEK_CUR);
        $this->debug(3, sprintf("serveCacheFile: image initial bytes mark it as %s. ngoffset = %u, ftell = %u, end of security block: %u", $imgType, $ngOffset, ftell($fp), strlen($this->filePrependSecurityBlock) + 6 + $ngOffset));
        if (ftell($fp) != strlen($this->filePrependSecurityBlock) + 6 + $ngOffset) {
            @unlink($this->cachefile);
            return $this->error("The cached image file seems to be corrupt.");
        }
        $imageDataSize = filesize($this->cachefile) - (strlen($this->filePrependSecurityBlock) + 6 + $ngOffset);
        ...
        if ($content != false) {
            $content = substr($content, strlen($this->filePrependSecurityBlock) + 6 - $ngOffset);
            echo $content;
        ...
        switch ($mimeType) {
            ...
            case 'image/avif':
                if (function_exists('imagecreatefromavif')) {
                    ...
                    $this->error("Creating from AVIF is unsupported by the server.");
                }
                break;
    ...
    protected function getURL($url, $tempfile)
    {
        ...
        if (function_exists('curl_init')) {
            ...
            // Makes it look like a browser, some hosts dislike us otherwise
            ...
            // For webp/avif, this needs to explicitly state that it can take such format
            $accepts = [
                'image/webp',
                '*/*'
            ];
            if (function_exists('imageavif')) {
                array_unshift($accepts, 'image/avif');
            }
